...
const PublicRoute = ({ user, loading }) => {
  if (loading) return <div className="text-center">Loading...</div>;
  return user ? <Navigate to="/employees" replace /> : <Outlet />;
};

// 🔁 Fallback
const RoleAwareFallback = ({ user }) => {
  return user ? <Navigate to="/employees" replace /> : <Navigate to="/login" replace />;
};
...
export default function RouterWrapper() {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/auth/operator/me", {
      credentials: "include",
    })
      .then((res) => res.json())
      .then((data) => {
