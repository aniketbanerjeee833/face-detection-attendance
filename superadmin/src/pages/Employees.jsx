...
export default function Employees() {
  ...
  const selectedStation = stationsList.find((s) => String(s.id) === stationFilter);

  const data       = isSuperAdmin ? superData : null;     
  const isLoading  = isSuperAdmin ? superLoading  : false; 
  const isFetching = isSuperAdmin ? superFetching  : false;
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-50">
              {employees.length === 0 ? (
                <tr>
                  <td colSpan={isSuperAdmin ? 6 : 7} className="px-6 py-10 text-center text-sm text-slate-400">
